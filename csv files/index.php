...
// Manual addition crop_code_name('Q', 'Quince');
    ...
    for($in_row=1; $in_row<count($csv2[$row]); $in_row++){
       //Check error if data is a crop code
        if(intval($csv2[$row][$in_row])){
            fwrite($error_log_file, "There is an Error on line  : ".($row+1)." , ". $csv2[$row][$in_row]. " it's not crop code. \n");
            $in_row = $in_row+1;
            continue;
        }elseif ($csv2[$row][$in_row] == "" || $csv2[$row][$in_row] == ","){
            $in_row = $in_row+1;
            continue;
        }

        //Display crop name instead of crop code
